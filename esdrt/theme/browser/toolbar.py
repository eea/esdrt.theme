"""plone.toolbar ViewletManager override."""
...
class ToolbarViewletManager(toolbar.ToolbarViewletManager):
    """Override toolbar to render list of roles."""
        ...
        for group in groups:
            if group.startswith("extranet-esd-ghginv-sr-"):
                ...
                if len(splitted_name) == 2:
                    sector_review_roles.append(
                        "%s - %s"
                        % (
                            self.get_country_name(splitted_name[1]),
                            splitted_name[0],
                        )
                    )

            elif group.startswith("extranet-esd-ghginv-qualityexpert-"):
                new_name = group.replace(
